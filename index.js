...
function getPuns() {
    var puns = punList.puns;
    var randOutput = puns[Math.floor(Math.random() * puns.length)];
    return randOutput;
}
...
bot.on('message', function (user, userID, channelID, message, evt) {
    ...
    } else if (message.toLowerCase().indexOf("!dadjoke") > -1 && user != "puzzle-bot" && respond) {
        //Get a random dad joke
        bot.sendMessage({to: channelID, message: getDadJoke()});
    } else if (message.toLowerCase().indexOf("!puns") > -1 && user != "puzzle-bot" && respond) {
        //Get a random dad joke
        bot.sendMessage({to: channelID, message: getPuns()});
    }
    //stop the pain
    else if (message.toLowerCase().indexOf("!puzzlebotstop") > -1 && user != "puzzle-bot" && respond) {
        bot.sendMessage({
            to: channelID,
            message: "There is not stopping puzzletron.  Just kidding.  \nType '!puzzlebotsAssemble' to start me again."
        });
        respond = false;
    }
    //i'm a glutton lets get dad bot back
    else if (message.toLowerCase().indexOf("!puzzleotstart") > -1 && user != "puzzle-bot" && !respond) {
        bot.sendMessage({
            to: channelID,
            message: "You've got to be dadding me.\nType '!puzzlebotStop' to stop dad facts. \n\n" + getDadJoke()
        });
        respond = true;
    }
    //dadbot briefing
    else if (message.toLowerCase().indexOf("!puzzlebot") > -1 && user != "puzzle-bot" && respond) {
        bot.sendMessage({
            to: channelID,
